...
Config file: ~/dotfiles/zsh/fancy-prompts-zsh.conf
End_Usage
}
...
_fancy-prompt() {
    ...
    if [ -n "$bright" ]; then
        ...
        local time_co=$yellow   host_co=$lt_blue  path_co=$magenta title_co=$lt_red at_co=$white
    else
...
"
    ...
    local conf_file=$HOME/dotfiles/zsh/fancy-prompts-zsh.conf
    if [ -r "$conf_file" ]; then
